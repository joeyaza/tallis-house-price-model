...
export default class DataExtractor {
    ...
    async extractFromPage(id, el) {
        const $ = cheerio.load(el), { data: propertyData } = await axios.get($("a").attr('href')), $$ = cheerio.load(propertyData), soldPrice = $$("span.sold-property__price-value")
            ...
            .text()
            .replace("kr", "")
            .replace(/\s+/g, ''), houseType = $$("dd.sold-property__attribute-value")
            .eq(3)
            .text()
            .replace(/\s+/g, ''), assignment = $$("dd.sold-property__attribute-value")
            .eq(4)
            .text()
            .replace(/\s+/g, ''), rooms = $$("dd.sold-property__attribute-value")
            .eq(5)
            .text()
            .replace('rum', "")
            .replace(/\s+/g, ''), livingArea = $$("dd.sold-property__attribute-value")
            .eq(6)
            .text()
            .replace("m²", '')
            .replace(/\s+/g, ''), extraArea = $$("dd.sold-property__attribute-value")
            .eq(7)
            .text()
            .replace("m²", '')
            .replace(/\s+/g, ''), land = $$("dd.sold-property__attribute-value")
            .eq(8)
            .text()
            .replace("m²", '')
